...
const StoryGalaxy: React.FC<StoryGalaxyProps> = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < filteredNodes.length; i += 10) {
      ...
      const x1 =
        centerX +
        (node1.x * Math.cos(rotation.y) - node1.z * Math.sin(rotation.y)) *
          zoom;
      const y1 =
        centerY +
        (node1.y * Math.cos(rotation.x) - node1.z * Math.sin(rotation.x)) *
          zoom;
      const x2 =
        centerX +
        (node2.x * Math.cos(rotation.y) - node2.z * Math.sin(rotation.y)) *
          zoom;
      const y2 =
        centerY +
        (node2.y * Math.cos(rotation.x) - node2.z * Math.sin(rotation.x)) *
          zoom;
    ...
    filteredNodes.forEach(node => {
      const x =
        centerX +
        (node.x * Math.cos(rotation.y) - node.z * Math.sin(rotation.y)) * zoom;
      const y =
        centerY +
        (node.y * Math.cos(rotation.x) - node.z * Math.sin(rotation.x)) * zoom;
    ...
    if (selectedNode) {
      const x =
        centerX +
        (selectedNode.x * Math.cos(rotation.y) -
          selectedNode.z * Math.sin(rotation.y)) *
          zoom;
      const y =
        centerY +
        (selectedNode.y * Math.cos(rotation.x) -
          selectedNode.z * Math.sin(rotation.x)) *
          zoom;
  ...
  if (isLoading) {
    return (
      <div
        className={`relative bg-slate-900 rounded-3xl overflow-hidden ${className}`}
      >
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-gray-900">
                {selectedNode.connections} related stories
              </p>
            </div>
            ...
            <div>
              ...
              >
                {selectedNode.impact.charAt(0).toUpperCase() +
                  selectedNode.impact.slice(1)}
              </span>
            </div>
          </div>
