...
interface SecurityBadgeProps {
  variant?: 'encryption' | 'privacy' | 'certified' | 'secure' | 'certification';
  text?: string;
  onClick?: () => void;
  className?: string;
...
const SecurityBadge: React.FC<SecurityBadgeProps> = ({
  ...
  return (
    <Component
      className={`inline-flex items-center gap-2 text-sm text-gray-600 ${onClick ? 'hover:text-gray-900 transition-colors cursor-pointer' : ''} ${backgroundClass} ${className}`}
      onClick={onClick}
      type={onClick ? 'button' : undefined}
    >
      <div className="text-gray-400">
        {icons[variant] || icons.certified}
      </div>
