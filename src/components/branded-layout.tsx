...
import { brandingManager, BrandingConfig, WhiteLabelConfig } from '@/lib/branding';
...
export function BrandedLayout({ projectId, children, className = '' }: BrandedLayoutProps) {
  ...
  const loadBranding = async () => {
    try {
      const response = await fetch(`/api/projects/${projectId}/branding?include_css=true`);
  ...
  if (brandingState.error) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-red-600 mb-2">Branding Load Error</h2>
  ...
  return (
    ...
    >
      ...
      {brandingState.branding?.cultural_patterns?.enabled && brandingState.branding.cultural_patterns.pattern_url && (
        <div 
...
function BrandedHeader({ branding, whitelabel, projectId }: BrandedHeaderProps) {
  const navigation_style = branding?.navigation_style || 'horizontal';

  const headerClasses = navigation_style === 'sidebar' 
    ? 'fixed left-0 top-0 h-full w-64 bg-brand-primary text-white shadow-lg z-20'
    : 'bg-brand-primary text-white shadow-lg';

  return (
    <header className={headerClasses}>
      <div className={navigation_style === 'sidebar' ? 'p-6' : 'container mx-auto px-4 py-6'}>
        <div className={navigation_style === 'sidebar' ? 'space-y-6' : 'flex items-center justify-between'}>
          {/* Logo/Title */}
          <div className={navigation_style === 'sidebar' ? 'text-center' : 'flex items-center space-x-4'}>
            {branding?.logo_url ? (
              <img 
                src={branding.logo_url} 
                alt={whitelabel?.platform_name} 
                className={navigation_style === 'sidebar' ? 'h-16 w-auto mx-auto mb-4' : 'h-10 w-auto'}
              />
            ) : (
              <h1 className={`font-bold font-brand-heading ${navigation_style === 'sidebar' ? 'text-xl' : 'text-2xl'}`}>
                {whitelabel?.platform_name || 'Community Stories'}
              </h1>
            )}
            
            {whitelabel?.tagline && navigation_style !== 'sidebar' && (
              <p className="text-sm opacity-90 hidden md:block">{whitelabel.tagline}</p>
            )}
          </div>

          {/* Navigation */}
          <nav className={navigation_style === 'sidebar' ? 'space-y-4' : 'hidden md:flex space-x-6'}>
            ...
            <NavLink href={`/projects/${projectId}/submit`}>Share Story</NavLink>
          </nav>
        ...
        {whitelabel?.tagline && navigation_style === 'sidebar' && (
          <p className="text-sm opacity-90 text-center mt-2">{whitelabel.tagline}</p>
        )}
      </div>
    </header>
  );
...
function BrandedFooter({ branding, whitelabel, projectId }: BrandedFooterProps) {
  return (
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            {whitelabel?.about_text && (
              <p className="text-sm text-gray-600 mb-3">{whitelabel.about_text}</p>
            )}
            <p className="text-xs text-gray-500">
              Powered by <span className="brand-primary font-semibold">Empathy Ledger</span>
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex space-x-4">
              {whitelabel?.social_links?.website && (
                <SocialLink href={whitelabel.social_links.website}>Website</SocialLink>
              )}
              {whitelabel?.social_links?.facebook && (
                <SocialLink href={whitelabel.social_links.facebook}>Facebook</SocialLink>
              )}
              {whitelabel?.social_links?.twitter && (
                <SocialLink href={whitelabel.social_links.twitter}>Twitter</SocialLink>
              )}
              {whitelabel?.social_links?.instagram && (
                <SocialLink href={whitelabel.social_links.instagram}>Instagram</SocialLink>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="border-t mt-8 pt-8 text-center text-sm text-gray-500">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
            <p>
              {whitelabel?.footer_text || `© ${new Date().getFullYear()} ${whitelabel?.platform_name}. All rights reserved.`}
            </p>
            <div className="flex space-x-4">
              {whitelabel?.privacy_policy_url && (
                <a href={whitelabel.privacy_policy_url} className="hover:underline">Privacy Policy</a>
              )}
              {whitelabel?.terms_of_service_url && (
                <a href={whitelabel.terms_of_service_url} className="hover:underline">Terms of Service</a>
              )}
              <a href="/sovereignty" className="hover:underline">Data Sovereignty</a>
            </div>
          </div>
        </div>
      </div>
