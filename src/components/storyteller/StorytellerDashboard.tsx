...
import { getUserProfile, getUserCommunities } from '@/lib/supabase-auth';
...
export default function StorytellerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'stories' | 'communities' | 'insights' | 'privacy'
  >('overview');
  ...
  const calculateStats = (
    ...
    const totalViews = userStories.reduce(
      (sum, story) => sum + story.view_count,
      0
    );
    const totalReactions = userStories.reduce(
      (sum, story) => sum + story.reaction_count,
      0
    );
        ...
        <div className="content-container">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                  <div className="bg-white border border-gray-100 rounded-3xl p-6 hover-lift">
                    ...
                    <div className="text-3xl font-extralight text-gray-900 mb-2">
                      {stats.published_stories}
                    </div>
                    <p className="text-sm text-gray-500 font-light">
                      Stories reaching the community
                    </p>
                  </div>
                  ...
                  {stories.slice(0, 3).map(story => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <h4 className="font-normal text-gray-900 flex-1">
                          {story.title}
                        </h4>
                        <span
                          className={`text-xs px-2 py-1 rounded-full font-light ${getStatusColor(story.status)}`}
                        >
                          {story.status}
                        </span>
                      </div>
              ...
              <div>
                ...
                <div className="space-y-4 mb-8">
                  <Link
                    href="/submit"
                    className="block bg-gray-900 text-white p-4 rounded-2xl hover:bg-gray-800 smooth-transition no-underline"
                  >
                  ...
                  <Link
                    href="/communities"
                    className="block bg-gray-50 text-gray-700 p-4 rounded-2xl hover:bg-gray-100 smooth-transition no-underline"
                  >
                    <div className="flex items-center gap-3">
                      <svg
                        className="w-5 h-5"
                  ...
                  {communities.slice(0, 3).map(membership => (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500 font-light">
                        ...
                        {membership.communities.story_count} stories
                      </p>
                    </div>
                  ))}
          ...
          {activeTab === 'stories' && (
            <div>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                <h2 className="text-2xl font-extralight text-gray-900 mb-4 sm:mb-0">
                  My Stories
                </h2>
              ...
              <div className="grid gap-6">
                {filterStories(stories).map(story => (
                  <div
                    key={story.id}
                    className="bg-white border border-gray-100 rounded-3xl p-8 hover-lift"
                  >
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`text-xs px-3 py-1 rounded-full font-light ${getStatusColor(story.status)}`}
                          >
                            {story.status}
                          </span>
                        </div>
                      ...
                      {(story.audio_url ||
                        ...
                        <div className="flex-shrink-0 ml-6">
                          <MediaDisplay
                            src={
                              story.image_urls?.[0] || placeholderImages.story
                            }
                            alt="Story media"
                            aspectRatio="square"
                            rounded="2xl"
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-3">
                        <Link
                          ...
                          className="text-gray-600 hover:text-gray-900 smooth-transition no-underline"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
          ...
          {activeTab === 'insights' && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 font-light">
                Personalized insights about your story impact coming soon.
              </p>
            </div>
          )}
          ...
          {activeTab === 'privacy' && user && (
            <PrivacySettingsPanel
              ...
              onSettingsUpdate={settings => {
                ...
                setProfile(prev =>
                  prev ? { ...prev, privacy_settings: settings } : null
                );
              }}
            />
          )}
