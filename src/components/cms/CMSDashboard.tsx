        ...
        <div className="max-w-7xl mx-auto px-8 md:px-12">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {metrics.map(metric => (
              <div
                key={metric.id}
                className="bg-white border border-gray-100 rounded-3xl p-8 text-center hover:shadow-lg transition-all duration-300"
              >
            ...
            <div className="grid lg:grid-cols-2 gap-16">
              ...
              <div className="bg-white rounded-3xl p-8">
                ...
                <div className="space-y-4">
                  {Object.entries(analytics.stories_by_category).map(
                    ([category, count]) => (
                      <div
                        key={category}
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-20 bg-gray-100 rounded-full h-2">
                            <div
                              ...
                              style={{
                                width: `${(count / Math.max(...Object.values(analytics.stories_by_category))) * 100}%`,
                              }}
                            ></div>
                          </div>
        ...
        <div className="max-w-7xl mx-auto px-8 md:px-12">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {insights.map(insight => (
              <div
                key={insight.id}
                className="bg-white border border-gray-100 rounded-3xl p-8 hover:shadow-lg transition-all duration-300"
              >
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 font-light">
                      {insight.location} • {insight.story_count} stories •{' '}
                      {insight.affected_population.toLocaleString()} people
                      affected
                    </p>
                  </div>
                ...
                {insight.policy_impact && insight.policy_impact.length > 0 && (
                  <div className="bg-green-50 rounded-2xl p-4">
                    <h4 className="text-sm font-normal text-green-900 mb-2">
                      Policy Impact
                    </h4>
                    <div className="space-y-1">
                      {insight.policy_impact.map((impact, index) => (
                        <p
                          key={index}
                          className="text-sm text-green-700 font-light"
                        >
                          {impact}
                        </p>
                      ))}
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${
                        story.sentiment_score && story.sentiment_score > 0.7
                          ? 'bg-green-500'
                          : story.sentiment_score && story.sentiment_score > 0.4
                            ? 'bg-yellow-500'
                            : 'bg-red-500'
                      }`}
                    ></div>
                    <span className="text-xs text-gray-500 font-light">
                      {story.sentiment_score
                        ? Math.round(story.sentiment_score * 100)
                        : 0}
                      % positive
                    </span>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {story.themes.map(theme => (
                    <span
                      key={theme}
                      className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded-full font-light"
                    >
                      {theme}
                    </span>
                  ))}
                ...
                {story.impact_metrics && (
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                    {Object.entries(story.impact_metrics).map(
                      ([key, value]) => (
