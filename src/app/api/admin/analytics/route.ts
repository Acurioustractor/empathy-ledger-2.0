...
export async function GET(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          sovereignty_note:
            'Cross-project analytics require platform-level responsibility',
        },
        { status: 403 }
      );
    }
    ...
    const include_project_breakdown =
      url.searchParams.get('breakdown') === 'true';
    ...
    const { analytics: cross_project_analytics, error: analytics_error } =
      await projectOperations.getCrossProjectAnalytics(
        user.id,
        time_period as any
      );
    ...
    const sovereignty_metrics = await getSovereigntyMetrics(
      supabase,
      time_period
    );
    ...
    const response_data = {
      ...
      sovereignty_principles: {
        consent_based_aggregation:
          'All metrics respect individual story consent settings',
        project_sovereignty_maintained:
          'Projects maintain control over their data sharing',
        community_empowerment_focused:
          'Analytics highlight community strengths and impact',
        transparency_commitment:
          'Communities can see how their data contributes to insights',
      },
      analysis_period: {
        period: time_period,
    ...
    if (include_project_breakdown) {
      (response_data as any).project_breakdown = await getProjectBreakdown(
        supabase,
        time_period
      );
    }
    ...
    if (include_trends) {
      (response_data as any).trends = await getTrendAnalysis(
        supabase,
        time_period
      );
    }
...
async function getPlatformOverview(supabase: any, period: string) {
  ...
  const { data: projects } = await supabase
    .from('projects')
    .select(
      'id, status, subscription_tier, created_at, total_stories, total_storytellers'
    )
    .eq('status', 'active');
  ...
  return {
    ...
    total_stories_platform:
      projects?.reduce((sum, p) => sum + (p.total_stories || 0), 0) || 0,

    active_storytellers_period:
      new Set(storytellers?.map(s => s.storyteller_id)).size || 0,
    total_storytellers_platform:
      projects?.reduce((sum, p) => sum + (p.total_storytellers || 0), 0) || 0,

    insights_generated: insights?.length || 0,

    subscription_tiers: {
      community:
        projects?.filter(p => p.subscription_tier === 'community').length || 0,
      organization:
        projects?.filter(p => p.subscription_tier === 'organization').length ||
        0,
      enterprise:
        projects?.filter(p => p.subscription_tier === 'enterprise').length || 0,
    },

    geographic_distribution: await getGeographicDistribution(
      supabase,
      period_start
    ),
...
async function getSovereigntyMetrics(supabase: any, period: string) {
  ...
  const stories_with_consent =
    stories?.filter(
      s => s.consent_settings && Object.keys(s.consent_settings).length > 0
    ).length || 0;

  const consent_categories = {
    allowAnalysis:
      stories?.filter(s => s.consent_settings?.allowAnalysis).length || 0,
    allowCommunitySharing:
      stories?.filter(s => s.consent_settings?.allowCommunitySharing).length ||
      0,
    allowResearch:
      stories?.filter(s => s.consent_settings?.allowResearch).length || 0,
    allowValueCreation:
      stories?.filter(s => s.consent_settings?.allowValueCreation).length || 0,
    allowPolicyUse:
      stories?.filter(s => s.consent_settings?.allowPolicyUse).length || 0,
  };

  // Calculate compliance scores
  const compliance_scores =
    projects?.map(p => p.sovereignty_compliance_score || 100) || [];
  const average_compliance =
    compliance_scores.length > 0
      ? compliance_scores.reduce((sum, score) => sum + score, 0) /
        compliance_scores.length
      : 100;

  return {
    average_compliance_score: Math.round(average_compliance),
    excellent_compliance:
      projects?.filter(p => (p.sovereignty_compliance_score || 100) >= 90)
        .length || 0,
    good_compliance:
      projects?.filter(p => {
    ...
    needs_attention:
      projects?.filter(p => (p.sovereignty_compliance_score || 100) < 70)
        .length || 0,

    consent_adherence: {
      total_stories: stories?.length || 0,
      stories_with_consent,
      adherence_rate: stories?.length
        ? Math.round((stories_with_consent / stories.length) * 100)
        : 100,
      consent_breakdown: consent_categories,
    },

    cultural_protocols: {
      stories_with_protocols:
        stories?.filter(
          s =>
      ...
      indigenous_data_sovereignty:
        projects?.filter(
          p => p.sovereignty_framework?.indigenous_data_sovereignty
        ).length || 0,
    },
    ...
    privacy_distribution: {
      ...
      community:
        stories?.filter(s => s.privacy_level === 'community').length || 0,
...
async function getImpactMetrics(supabase: any, period: string) {
  ...
  const { data: value_events } = await supabase
    .from('value_events')
    .select(
      'event_type, value_amount, storyteller_share, community_share, occurred_at'
    )
  ...
  const total_value_created =
    value_events?.reduce((sum, event) => sum + (event.value_amount || 0), 0) ||
    0;
  const community_value_share =
    value_events?.reduce(
      (sum, event) => sum + (event.community_share || 0),
      0
    ) || 0;
  const storyteller_value_share =
    value_events?.reduce(
      (sum, event) => sum + (event.storyteller_share || 0),
      0
    ) || 0;

  // Group value events by type
  const value_by_type =
    value_events?.reduce((acc, event) => {
  ...
  return {
    financial_impact: {
      ...
      value_sharing_rate:
        total_value_created > 0
          ? Math.round(
              ((community_value_share + storyteller_value_share) /
                total_value_created) *
                100
            )
          : 0,
    ...
    policy_influence: {
      policy_citations:
        value_events?.filter(e => e.event_type === 'policy_influence').length ||
        0,
      advocacy_uses:
        value_events?.filter(e => e.event_type === 'advocacy_use').length || 0,
    },

    grants_and_funding: {
      grants_received:
        value_events?.filter(e => e.event_type === 'grant_funded').length || 0,
      total_grant_amount:
        value_events
    ...
    media_and_outreach: {
      media_features:
        value_events?.filter(e => e.event_type === 'media_license').length || 0,
      public_presentations:
        value_events?.filter(e => e.event_type === 'public_presentation')
          .length || 0,
    },
    ...
    actionable_insights: {
      ...
      policy_insights:
        actionable_insights?.filter(
          i => i.insight_type === 'policy_recommendation'
        ).length || 0,
      community_insights:
        actionable_insights?.filter(
          i => i.insight_type === 'community_strength'
        ).length || 0,
    },
  };
}
  ...
  return {
    ...
    engagement_metrics: {
      stories_per_project: current_projects?.length
        ? Math.round((current_stories?.length || 0) / current_projects.length)
        : 0,
      stories_per_storyteller: new Set(
        current_storytellers?.map(s => s.storyteller_id)
      ).size
        ? Math.round(
            (current_stories?.length || 0) /
              new Set(current_storytellers?.map(s => s.storyteller_id)).size
          )
        : 0,
    },
  };
  ...
  return (
    projects?.map(project => ({
      ...
      activity_level: getActivityLevel(
        project.total_stories,
        project.active_storytellers
      ),
      compliance_status: getComplianceStatus(
        project.sovereignty_compliance_score
      ),
...
function getGrowthIndicator(last_story: string | null): string {
  ...
  const daysSince = Math.floor(
    (Date.now() - new Date(last_story).getTime()) / (1000 * 60 * 60 * 24)
  );
