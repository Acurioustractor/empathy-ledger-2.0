  ...
  try {
      ...
      return NextResponse.json(
        {
          error,
          sovereignty_note:
            'Access to project details requires membership and appropriate permissions',
        },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({
      ...
      sovereignty_principles: {
        community_ownership:
          'This project belongs to the participating community',
        storyteller_control:
          'Storytellers maintain ownership and control over their narratives',
        cultural_protocols:
          'Community-defined protocols are technically enforced',
  ...
  try {
    ...
    if (updates.sovereignty_framework) {
      ...
      const missing_keys = required_keys.filter(
        key => !(key in updates.sovereignty_framework)
      );
        ...
        return NextResponse.json(
          {
            ...
            missing_keys,
            sovereignty_note:
              'All projects must maintain minimum sovereignty standards',
          },
          { status: 400 }
        );
      }
    ...
    if (!success) {
      return NextResponse.json(
        {
          error,
          sovereignty_note: 'Updates must maintain sovereignty compliance',
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      message:
        'Project configuration updated successfully while maintaining sovereignty principles',
    });
  } catch (error: any) {
    return NextResponse.json(
      { error: 'Internal server error', message: error.message },
      ...
      return NextResponse.json(
        {
          ...
          sovereignty_note:
            'Project deletion requires the highest level of community authorization',
        },
        { status: 403 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          story_count,
          sovereignty_note:
            'Community stories must be preserved - consider archiving instead of deletion',
          alternative:
            'Use project archiving to preserve stories while disabling access',
        },
        { status: 409 }
      );
    }
