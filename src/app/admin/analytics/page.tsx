    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-red-600 mb-2">Analytics Load Error</h2>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600">
                  Platform-wide insights that serve community empowerment while respecting sovereignty
                </p>
              </div>
            ...
            <div className="mt-4 flex items-center space-x-6 text-sm text-gray-500">
              ...
              <span>Last Updated: {new Date(analytics.analysis_period.last_updated).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-8">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-blue-900 mb-3">Sovereignty & Ethics Framework</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
              <div>
                <strong>Consent-Based:</strong> {analytics.sovereignty_principles.consent_based_aggregation}
              </div>
              <div>
                <strong>Project Sovereignty:</strong> {analytics.sovereignty_principles.project_sovereignty_maintained}
              </div>
              <div>
                <strong>Community Empowerment:</strong> {analytics.sovereignty_principles.community_empowerment_focused}
              </div>
              <div>
                <strong>Transparency:</strong> {analytics.sovereignty_principles.transparency_commitment}
              </div>
            </div>
          </div>
          ...
          {analytics.cross_project_analytics && (
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Cross-Project Insights</h3>
              <div className="text-sm text-gray-600 mb-4">
                Aggregated insights from projects that have opted into cross-project collaboration
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {analytics.cross_project_analytics.map((item: any, index: number) => (
                  <div key={index} className="bg-gray-50 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 mb-2">{item.project?.name}</h4>
                    <div className="space-y-2 text-sm text-gray-600">
                      <div>Stories: {item.stories_submitted || 0}</div>
                      <div>Storytellers: {item.active_storytellers || 0}</div>
                      <div>Engagement: {item.community_engagement_score || 0}%</div>
                    </div>
                  </div>
                ))}
              </div>
...
function ActionItems({ analytics }: ActionItemsProps) {
  const generateActionItems = () => {
    ...
    // Engagement
    if (analytics.platform_overview.active_storytellers_period < analytics.platform_overview.total_storytellers_platform * 0.1) {
      items.push({
        type: 'info',
        title: 'Community Engagement',
        message: 'Active storyteller engagement is low relative to total community',
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-6">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-lg font-semibold text-green-900">Platform Health Excellent</h3>
            <p className="text-green-800">All key metrics are performing well. Continue monitoring for optimal community empowerment.</p>
          </div>
        </div>
      </div>
    );
        ...
        {actionItems.map((item, index) => (
          <div 
            ...
            className={`border-l-4 pl-4 py-3 ${
              item.type === 'warning' ? 'border-yellow-400 bg-yellow-50' :
              item.type === 'error' ? 'border-red-400 bg-red-50' :
              'border-blue-400 bg-blue-50'
            }`}
          >
            <h4 className="font-medium text-gray-900">{item.title}</h4>
            <p className="text-sm text-gray-600 mt-1">{item.message}</p>
            <p className="text-sm font-medium text-gray-800 mt-2">Action: {item.action}</p>
          </div>
        ))}
      </div>
    </div>
