/**
 * EMPATHY LEDGER ORGANISATION INSIGHTS SYSTEM
 * Advanced analytics and value dashboard for organisations
 */
...
/**
 * Generate comprehensive organisational insights
 */
export async function generateOrganisationInsights(
  organisationId: string,
  periodStart: string,
  periodEnd: string,
  userId: string
): Promise<{ insights: OrganisationInsights | null; error: any }> {
  try {
    // Check permissions
    ...
    // Get organisation stories for the period
    ...
    const insights: OrganisationInsights = {
 ...
 */
async function storeInsights(insights: OrganisationInsights): Promise<void> {
  try {
    await supabase.from('organisation_insights').insert({
      organisation_id: insights.organisation_id,
...
/**
 * Get cached insights for an organisation
 */
...
export async function getInsightsSummary(
  ...
  try {
    ...
    if (!insights) {
      ...
      const { insights: freshInsights } = await generateOrganisationInsights(
        organisationId,
        startDate,
        endDate,
        userId
 ...
 */
function createInsightsSummary(insights: OrganisationInsights) {
  return {
