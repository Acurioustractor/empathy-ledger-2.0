...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const enrichUserWithCommunityData = async (
    authUser: User
...
export function useAuth() {
  ...
  if (context === undefined) {
    throw new Error(
      'useAuth must be used within an AuthProvider - this means the component is not wrapped in the community authentication context'
    );
  }
  return context;
}
...
export function useCommunityMember() {
  ...
  return {
    ...
    fullName:
      user?.storyteller_profile?.full_name || user?.user_metadata?.full_name,
