  ...
  try {
    ...
    if (story.privacy_level === 'public') {
      ...
      const culturalProtocols =
        story.cultural_protocols as CulturalProtocols | null;
    ...
    if (story.privacy_level === 'community') {
      ...
      if (userId) {
        ...
        return (
          user?.community_affiliation ===
          (story.storyteller as any)?.community_affiliation
        );
      }
    }
...
export async function submitStory(
  ...
  try {
    ...
    if (
      !storyData.consent_settings.allowAnalysis &&
      storyData.privacy_level !== 'private'
    ) {
      throw new Error(
        'Public or community stories require analysis consent for community insights'
      );
    }
    ...
    if (storyData.consent_settings.allowAnalysis) {
      ...
      console.log(
        'Story submitted with analysis consent - triggering analysis queue'
      );
    }
  ...
  try {
    ...
    const { data: analysis, error } = await supabase
      ...
      .insert({
        ...
        cultural_review_status: story.cultural_protocols
          ? 'pending'
          : 'approved',
      })
      .select()
...
export async function generateCommunityInsights(
  ...
  try {
    ...
    if (validatedStories.length < 3) {
      throw new Error(
        'Insufficient validated stories for community insights (minimum 3 required)'
      );
    }
...
export async function trackValueEvent(
  ...
  try {
    ...
    const storytellerShare = valueEventData.value_amount
      ? valueEventData.value_amount * 0.4
      : 0;
    const communityShare = valueEventData.value_amount
      ? valueEventData.value_amount * 0.4
      : 0;
...
export async function getStorytellerDashboard(
  ...
  try {
    ...
    const analyzedStories =
      stories?.filter(s => s.analysis?.length > 0).length || 0;
    const validatedStories =
      stories?.filter(s =>
    ...
    const totalValueGenerated =
      valueEvents?.reduce(
        (sum, event) => sum + (event.event_data as any).storyteller_share || 0,
        0
      ) || 0;
