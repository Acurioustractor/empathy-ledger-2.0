...
export function classifySupabaseError(error: any): SupabaseErrorInfo {
  ...
  if (errorMessage.includes('Failed to fetch') || 
    ...
    return {
      ...
      suggestion: 'Verify your internet connection and try again. If the problem persists, check if Supabase is experiencing issues.',
      retryable: true,
      statusCode,
      originalError: error
    };
  ...
  if (errorMessage.includes('Invalid login credentials') ||
    ...
    return {
      ...
      suggestion: 'Sign out and sign back in. If the problem persists, check your email for a confirmation link.',
      retryable: false,
      statusCode,
      originalError: error
    };
  ...
  if (errorMessage.includes('new row violates row-level security') ||
    ...
    return {
      ...
      suggestion: 'Contact your project administrator for access. This might be due to cultural protocol restrictions.',
      retryable: false,
      statusCode,
      originalError: error
    };
  ...
  if (errorMessage.includes('Too many requests') ||
    ...
    return {
      ...
      suggestion: 'Wait 60 seconds and try again. Consider reducing the frequency of your requests.',
      retryable: true,
      statusCode,
      originalError: error
    };
  ...
  if (errorMessage.includes('timeout') ||
    ...
    return {
      ...
      suggestion: 'Try again with a simpler query or check your internet connection.',
      retryable: true,
      statusCode,
      originalError: error
    };
  ...
  if (errorMessage.includes('duplicate key value') ||
    ...
    return {
      ...
      userMessage: 'The data you\'re trying to save conflicts with existing records.',
      suggestion: 'Check for duplicate entries or missing required fields and try again.',
      retryable: false,
      statusCode,
      originalError: error
    };
  ...
  return {
    ...
    suggestion: 'Please try again. If the problem persists, contact support with this error code.',
    retryable: true,
    statusCode,
    originalError: error
  };
...
export async function withErrorHandling<T>(
  ...
  try {
    ...
    const errorInfo = classifySupabaseError(error);
    console.error(`💥 Unexpected error in ${context || 'unknown context'}:`, error);
    throw new SupabaseError(errorInfo);
  }
}

// Helper for logging errors with context
export function logSupabaseError(error: any, context: string, additionalInfo?: any) {
  const errorInfo = classifySupabaseError(error);
...
export const ERROR_MESSAGES = {
  STORY_SAVE_FAILED: 'Failed to save your story. Please check your internet connection and try again.',
  PROJECT_ACCESS_DENIED: 'You don\'t have access to this project. Contact your project administrator.',
  LOGIN_FAILED: 'Login failed. Please check your email and password.',
  NETWORK_ISSUES: 'Connection issues detected. Please check your internet connection.',
  RATE_LIMITED: 'Too many requests. Please wait a moment and try again.',
  PERMISSION_DENIED: 'You don\'t have permission for this action. This might be due to cultural protocol restrictions.'
};
