...
export interface Story {
  ...
  audio_url?: string;
  video_url?: string;
  category: 'healthcare' | 'education' | 'housing' | 'youth' | 'elder-care' | 'policy';
...
export interface SiteMetric {
  ...
  metric_type: 'story_count' | 'community_count' | 'policy_changes' | 'value_created';
  description: string;
  last_updated: string;
...
export async function getStoriesByCategory(category: string, limit: number = 10): Promise<Story[]> {
  try {
    ...
    return data || mockStories.filter(story => story.category === category).slice(0, limit);
  } catch (error) {
    console.error('Error fetching stories by category:', error);
    // Return mock data filtered by category when Supabase is not available
    return mockStories.filter(story => story.category === category).slice(0, limit);
  }
}
...
export async function getStoryAnalytics(): Promise<{
  ...
  try {
    ...
    const storiesByCategory = categoryData?.reduce((acc, story) => {
...
export async function getCommunityInsights(limit: number = 6): Promise<CommunityInsight[]> {
  try {
    const { data, error } = await supabase
...
export async function getInsightsByLocation(location: string): Promise<CommunityInsight[]> {
  try {
    const { data, error } = await supabase
      .from('community_insights')
      .select('*')
...
export async function getPageContent(pageSlug: string): Promise<ContentBlock[]> {
  try {
    const { data, error } = await supabase
      .from('content_blocks')
      .select('*')
...
export async function updatePageContent(pageSlug: string, sectionName: string, contentData: any): Promise<boolean> {
  try {
...
export async function updateMetric(metricName: string, value: string | number): Promise<boolean> {
  try {
    const { error } = await supabase
      .from('site_metrics')
      .update({
...
export async function searchStories(query: string, filters?: {
  category?: string;
  location?: string;
...
export const mockStories: Story[] = [
  {
    ...
    content: 'After sharing my experience with mental health services, the community center implemented 24/7 support lines...',
    category: 'healthcare',
  ...
  },
  {
    ...
    content: 'Our voices led to culturally responsive teaching programs in three universities...',
    category: 'education',
    privacy_level: 'public',
...
export const mockInsights: CommunityInsight[] = [
  {
    ...
    description: 'Analysis of 347 stories reveals critical after-hours support needs',
    story_count: 347,
    ...
    policy_impact: ['City Council Motion 2024-15 approved', 'State funding allocation increased 40%'],
    category: 'healthcare',
    location: 'Brisbane',
