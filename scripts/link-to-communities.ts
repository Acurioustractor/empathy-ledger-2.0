...
async function linkToCommunities() {
  ...
  const { data: finalCheck } = await client
    ...
    .select(`
      ...
      communities!users_organization_id_fkey(name)
    `)
    .eq('role', 'storyteller')
  ...
  finalCheck?.slice(0, 8).forEach(storyteller => {
    ...
    console.log(`    Linked Community: ${storyteller.communities?.name}`)
  })

  // Show community distribution
  console.log(`\n📊 COMMUNITY DISTRIBUTION:`)
  const communityCount = new Map()
  finalCheck?.forEach(storyteller => {
    const communityName = storyteller.communities?.name || 'Unknown'
    communityCount.set(communityName, (communityCount.get(communityName) || 0) + 1)
  })

  [...communityCount.entries()]
    .sort(([,a], [,b]) => b - a)
...
linkToCommunities().catch(console.error)