#!/usr/bin/env tsx
/**
 * BULLETPROOF COMPLETE MIGRATION
 * Download images, capture transcripts, link everything correctly
 */
...
async function migrateStorytellersWithImagesAndTranscripts(): Promise<Map<string, string>> {
  console.log('\\n👥 MIGRATING STORYTELLERS WITH IMAGES & TRANSCRIPTS');
  console.log('===================================================');
  ...
  for (const record of airtableStorytellers) {
    try {
      ...
      // Create transcript record if we have transcript
      if (transcript) {
...
async function migrateStoriesWithImagesAndLinks(storytellerMap: Map<string, string>): Promise<void> {
...
async function validateMigrationResults(): Promise<void> {
...
async function main() {
  console.log('🚀 BULLETPROOF COMPLETE MIGRATION');
  console.log('==================================');
  console.log('Migrating storytellers + stories with images, transcripts, and proper linking');
  console.log('');
  
  try {
    // Step 1: Migrate storytellers with images and transcripts
    const storytellerMap = await migrateStorytellersWithImagesAndTranscripts();
    
    // Step 2: Migrate stories with images and proper storyteller linking
    await migrateStoriesWithImagesAndLinks(storytellerMap);
    ...
    console.log('\\n🎉 BULLETPROOF MIGRATION COMPLETED!');
    console.log('====================================');
    console.log('✅ All storytellers migrated with profile images');
